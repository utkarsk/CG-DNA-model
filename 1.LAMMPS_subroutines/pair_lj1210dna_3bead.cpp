...
/* --------------------------------------------------------------------------------------------------------------
   Contributing Author(s): Utkarsh Kapoor (Texas A&M), Young Chan Kim (NRL Navy), Jeetain Mittal (Texas A&M)
----------------------------------------------------------------------------------------------------------------- */
...
#include "stdlib.h"
#include "string.h"
#include "pair_lj1210dna_3bead.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
...
PairLj1210DNA_3Bead::PairLj1210DNA_3Bead(LAMMPS *lmp) : Pair(lmp) 
{
...
PairLj1210DNA_3Bead::~PairLj1210DNA_3Bead()
{
  if (allocated) {
...
void PairLj1210DNA_3Bead::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	// if in the same strand && adjcent to each other (1-2 interactions)
        if ((imol == jmol) && ((jtag == itag + 3) || (jtag == itag - 3))) {         
                if (q[itype][jtype] == 1) { // - on - only when they are suppose to stack
        ...
        else if ((imol == jmol) && ((jtag == itag + 6) || (jtag == itag - 6))) { // 1-3 always interact via excluded volume, irrespective of stacking or HB
...
void PairLj1210DNA_3Bead::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLj1210DNA_3Bead::settings(int narg, char **arg)
{
...
void PairLj1210DNA_3Bead::coeff(int narg, char **arg)
{
  ...
  double q_one = utils::numeric(FLERR,arg[6],false,lmp);
...
double PairLj1210DNA_3Bead::init_one(int i, int j)
{
...
void PairLj1210DNA_3Bead::write_restart(FILE *fp)
{
...
void PairLj1210DNA_3Bead::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
	...
	if (me == 0) {
	  ...
	  utils::sfread(FLERR,&sigma[i][j],sizeof(double),1,fp,NULL,error); 
          utils::sfread(FLERR,&q[i][j],sizeof(double),1,fp,NULL,error); 
...
void PairLj1210DNA_3Bead::write_restart_settings(FILE *fp)
{
...
void PairLj1210DNA_3Bead::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    utils::sfread(FLERR,&cut_global,sizeof(double),1,fp,NULL,error); 
    utils::sfread(FLERR,&offset_flag,sizeof(double),1,fp,NULL,error); 
    utils::sfread(FLERR,&mix_flag,sizeof(double),1,fp,NULL,error); 
    utils::sfread(FLERR,&tail_flag,sizeof(double),1,fp,NULL,error); 
  }
...
double PairLj1210DNA_3Bead::single(int i, int j, int itype, int jtype,
			...
			double &fforce)
{
  ...
  itag = atom->tag[i];
  jtag = atom->tag[j];
 //printf("Yes %d %d %d %d", imol, jmol, itag, jtag);
  ...
  fforce = factor_lj*forcesthb*r2inv;
          ...
          // if in the same strand && adjcent to each other (1-2 interactions)
          if ((imol == jmol) && ((jtag == itag + 3) || (jtag == itag - 3))) { // - on - only when they are suppose to stack
		if (q[itype][jtype] == 1) {
